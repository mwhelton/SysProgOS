		...
		switch( eir ) {
		   ...
		   case UA4_EIR_RX_INT_PENDING:
			...
			if( _reading.size > 0 ) {
				...
				status_t status = _q_deque( &_reading,
							(void **) &pcb );
				if( status != E_SUCCESS || pcb == NULL ) {
					_kpanic( "_sio_isr",
						 "read queue remove failed" );
				}
				// return value via EAX
				RET(pcb) = ch & 0xff;
				_schedule( pcb );
...
void _sio_init( void ) {
	...
	*/
	...
	/*
	** Reset the SIO "blocked for input" queue
	*/

	_q_reset( &_reading, NULL );
